...
static int min_width = 725;
static float menu_height_ratio = 2.0;
static int bh, mw, mh;
static int dmx = 300;
static int dmy = 16; /* put dmenu at these x and y offsets */
static unsigned int dmw = 130; /* make dmenu this wide */
 ...
 char *censort;
	...
	if (lines > 0) {
		...
		//	x = drawitem(item, x, 0, MIN(TEXTW(item->text), mw - x - TEXTW(">")));
			// x = drawitem(item, x, 0, textw(item->text));
			x = drawitem(item, x, 0, textw(item->text));
...
static int
textw_clamp(const char *text, unsigned int n)
{
    return drw_fontset_getwidth_clamp(drw, text, n) + lrpad;
}
...
setup(void)
{
...
#endif
	{
		...
		if (centered) {
			...
			/* set menu window width */
if (dmw > 0)
	mw = dmw;
else
	mw = sw;

/* set menu window x position */
if (centered)
	x = (sw - mw) / 2;
else if (dmx >= 0)
	x = dmx;
